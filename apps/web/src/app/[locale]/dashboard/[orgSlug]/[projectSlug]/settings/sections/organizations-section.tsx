      ...
      <CardContent className="space-y-4">
        {organizations && organizations.length > 0 ? (
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                {t("deleteDialogDescription", { name: org.name })}
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
          ...
          <div className="py-8 text-center text-muted-foreground">
            ...
            <p>{t("noOrgsTitle")}</p>
            <p className="text-sm">{t("noOrgsDescription")}</p>
          </div>
        )}
      </CardContent>
    </Card>
...
function MembersList({ organizationId, userRole }: { organizationId: string; userRole: string }) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <p className="text-sm font-medium">{t("membersCount", { count: members?.length || 0 })}</p>
      ...
      {members && members.length > 0 ? (
              ...
              <div className="flex items-center gap-2">
                ...
                {canManage && member.role !== "owner" && (
                  <Button
                    ...
                    onClick={() => {
                      if (confirm(t("removeMemberConfirm", { email: member.user?.email || "" }))) {
        ...
        <p className="py-4 text-center text-sm text-muted-foreground">{t("noMembersYet")}</p>
      )}
    </div>
  );
}
